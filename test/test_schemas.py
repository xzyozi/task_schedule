...
from modules.scheduler.schemas import ShellJobParams

# Minimal valid shell job params for testing the validator
def get_minimal_shell_params(**kwargs):
    base_config = {
        "task_type": "shell",
        "command": "echo test",
    }
...
def test_validate_cwd_valid():
    ...
    config = get_minimal_shell_params(cwd="my/relative/path")
    job_config = ShellJobParams.model_validate(config)
    assert job_config.cwd == "my/relative/path"


def test_validate_cwd_none():
    """Tests that a None cwd passes validation."""
    config = get_minimal_shell_params(cwd=None)
    job_config = ShellJobParams.model_validate(config)
...
def test_validate_cwd_rejects_absolute_posix():
    ...
    with pytest.raises(ValidationError, match='CWD must be a relative path'):
        config = get_minimal_shell_params(cwd="/an/absolute/path")
        ShellJobParams.model_validate(config)


def test_validate_cwd_rejects_absolute_windows():
    """Tests that an absolute Windows-style path is rejected."""
    with pytest.raises(ValidationError, match='CWD must be a relative path'):
        config = get_minimal_shell_params(cwd="C:\\an\\absolute\\path")
        ShellJobParams.model_validate(config)
...
def test_validate_cwd_rejects_traversal():
    ...
    with pytest.raises(ValidationError, match='CWD must be a relative path'):
        config = get_minimal_shell_params(cwd="../some/path")
        ShellJobParams.model_validate(config)
