...
for section in config.sections():
    ...
    print(f"セクション '{section}' のデータ: {process_data}")
...
# デバッグ情報の出力（既存のデータフレームを確認）
print("既存データフレーム:")
print(existing_df)

# 重複する'Executable Path'と'Arguments'を持つ行を除外
merged_df = pd.concat([existing_df, new_df], ignore_index=True)
deduplicated_df = merged_df.drop_duplicates(subset=['Executable Path', 'Arguments'], keep='first')

# デバッグ情報の出力（重複排除後のデータフレームを確認）
print("\n重複排除後のデータフレーム:")
