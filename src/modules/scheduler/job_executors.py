...
def _log_job_start(
    db: Session, job_id: str, command: str, workflow_run_id: Optional[int] = None
) -> models.ProcessExecutionLog:
...
def _execute_subprocess(
    ...
    if cwd:
        ...
        if absolute_cwd is None:
            ...
            return {
                "stdout": "",
                "stderr": f"Invalid cwd: '{cwd}' resolves outside of the sandboxed work directory.",
                "exit_code": 1,
            }
...
def execute_shell_job(
    ...
    try:
        ...
        try:
            ...
            result = _execute_subprocess(command_to_run=command_list, cwd=params.cwd, env=params.env)
...
def run_workflow(workflow_id: int, job_id: str = None, db: Optional[Session] = None):
    ...
    try:
        workflow = (
            db_session.query(models.Workflow)
            .options(joinedload(models.Workflow.steps))
        ...
        for i, step in enumerate(steps):
            ...
            try:
                ...
                step_result = None
                if task_type == "python":
                    step_result = execute_python_job(
                        job_id=step_job_id,
