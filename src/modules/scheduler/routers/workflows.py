...
from modules.scheduler import loader, models, schemas, service
...
def create_workflow(workflow_in: schemas.WorkflowCreate, db: Session = Depends(get_db)):
    ...
    if existing_workflow:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT, detail=f"Workflow with name '{workflow_in.name}' already exists."
        )
...
def get_workflow_run_logs(run_id: int, db: Session = Depends(get_db)):
    ...
    logs = (
        ...
        .filter(models.ProcessExecutionLog.workflow_run_id == run_id)
        .order_by(models.ProcessExecutionLog.start_time)
        .all()
    )
    if not logs:
